...
def test_on_morph():
    ...
    print("testing optimizing with side chain rotaton mode and madrax")
    s = run_optimization(test_pdb,test_map,rotate_side_chains=True,verbose=True, use_madrax=True)
    print("\tfinal score =", s)
...
def run_one_only():
    test_pdb = "croz/dataset/pdb_models/run1_morph2_job_8.pdb"
    test_map = "croz/dataset/crioEM_data/MORPHOLOGY_2_UNPROCESSED_MAP_cryosparc_P7_J418_Zflipped.mrc"
    s = run_optimization(test_pdb,test_map,verbose=True,rotateType="backbone",out_pdb="final.pdb",num_optimization_steps=100,use_madrax=True)
